...
- `startup`: empty file parse
- `compile bytecode`: `--asm` on a large asm file
- `disassemble`: `--disasm` on the generated bytecode
- `interpret big file`: parse + execute a large source file
- Cross-language loops and large literal lists for Disturb/Lua/Node/C if present
