                                ...
                                const char *match, size_t match_len)
{
    ...
    if (vm) {
        ...
        if (old_match && entry_is_string(old_match)) {
            old_match_len = papagaio_bytes_len(old_match->obj);
            old_match_buf = (char*)malloc(old_match_len + 1);
            if (old_match_buf) {
                memcpy(old_match_buf, papagaio_bytes_data(old_match->obj), old_match_len);
    ...
    size_t slen = 0;
    if (stringified) {
        data = papagaio_bytes_data(stringified->obj);
        slen = papagaio_bytes_len(stringified->obj);
    }
